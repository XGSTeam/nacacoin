...
#if defined(NDEBUG)
# error "NaCacoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "NaCacoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s NACA additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // NOW Subsidy is cut in half every 2,400,000 blocks which will occur approximately every 4 years.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
void ThreadScriptCheck() {
    RenameThread("nacacoin-scriptch");
...
// This function is not used anymore, it creates a warning. Commenting it.
